...
class IssueDetailLoader: Loader<Model.Comment> {
    ...
    override func bind() {
        issueChangedObservable = stateButtonTapSubject.switchLatest()
        ...
        postComment.flatMap { [number = self.issue.number] (comment) -> Observable<Model.Comment> in
            ...
            }).bind(to: datasourceIn).disposed(by: disposeBag)
