...
class IssueDetailViewController: UIViewController {
    ...
    lazy var loader: IssueDetailLoader =  { [unowned self] in
        return IssueDetailLoader(issue: self.issue)
        }()
...
extension IssueDetailViewController {
    ...
    func bind() {
        ...
        header.asObserver().skip(1).subscribe(onNext: { [weak self] (header) in
        ...
        RxKeyboard.instance.visibleHeight
            .drive(onNext: { [weak self] keyboardVisibleHeight in
                ...
                self.bottomConstraint.constant = actualKeyboardHeight
        ...
        loader.postDone.delay(0.3, scheduler: MainScheduler.instance).subscribe(onNext: { [weak self] _ in
            guard let `self` = self else { return }
            self.commentTextField.text = nil
            self.commentTextField.resignFirstResponder()
            let itemCount = self.collectionView.numberOfItems(inSection: 0)
            self.collectionView.scrollToItem(at: IndexPath(item: itemCount-1, section: 0), at: .bottom, animated: true)
        }).disposed(by: disposeBag)
    }
}
